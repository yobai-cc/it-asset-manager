...
## 2026-06-24：v1.5.1 内部重构与部署脚本修复

v1.5 之后的内部重构与修复。HTTP 契约与外部 API 行为不变。
...
- 部署脚本修复：`deploy.sh` 访问提示改为按 `gunicorn.conf.py` 实际 `bind` 渲染——默认仅监听 `127.0.0.1` 时提示「本机访问 + 配置反向代理」，`--port` 自定义端口绑 `0.0.0.0` 时显示局域网地址；rsync 排除 `android/` 目录。
